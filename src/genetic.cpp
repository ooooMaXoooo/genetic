...
/*
    Cette fonction fitness retourne la somme des coordonnées des vecteurs, le
tout au carré
*   dans notre cas, c'est la fonction x ⟼ x²
!   DO NOT CHANGE THE DEFINITION OF THE FUNCTION, CHANGE THE CODE INSIDE !
*/
inline real fitness(const std::array<Vec<Dimension>, NumberOfVectors>& vecteurs) {  //? la fonction fitness
  real sum = 0;
...
real eval_agent(const Agent& a) {
  ...
  // normalement à cette ligne, la variable vecteur représente correctement la
  // même information déjà stockée dans l'agent a
...
HalfPopulation selection_tournoi(const Population& p) {
  ...
  bool best_has_been_selectionned = false;  // on laisse comme ça pour le moment, on essai d'éviter la
                                            // convergence prématurée (c.f [1])
  for (size_t i = 0; i < HalfPopulationSize; i++) {
    // on prend 2 indices :
    int64_t iA = Randomizer::getInt(0, PopulationSize - 1);
    int64_t iB = Randomizer::getInt(0, PopulationSize - 1);

    // on prend le meilleur des 2 agents
    //? NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index
    real evalA = eval_agent(p[iA]);
    //? NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index
    real evalB = eval_agent(p[iB]);
...
static void bit_level_crossover(const Agent& p1, const Agent& p2, Agent& child1, Agent& child2, size_t num_elements,
                                size_t element_size_bits, GetterFunc getter, SetterFunc setter) {
  size_t indice_decoupe = Randomizer::getInt(0, static_cast<int64_t>(num_elements * element_size_bits) - 1);
...
std::pair<Agent, Agent> cross_over(const Agent& p1, const Agent& p2) {
  ...
  for (size_t chromo = 0; chromo < NumberOfVectors; chromo++) {
    ...
    auto getter = [chromo](const Agent& agent, size_t gene_idx) -> integer { return agent.getGene(chromo, gene_idx); };

    auto setter = [chromo](Agent& agent, size_t gene_idx, integer value) { agent.setGene(chromo, gene_idx, value); };

    bit_level_crossover(p1, p2, child1, child2, Dimension, Nb_bin, getter, setter);
  }

  // ========== CROSSOVER DU CHROMOSOME DES PROBAS ==========
  auto getter_proba = [](const Agent& agent, size_t idx) -> integer { return agent.getMutationProba(idx); };

  auto setter_proba = [](Agent& agent, size_t idx, integer value) { agent.setMutationProba(idx, value); };

  bit_level_crossover(p1, p2, child1, child2, NumberOfVectors + 1, Nb_bin, getter_proba, setter_proba);
  ...
  *p = cross_over_half_pop(hp);
  ...
  if (is_saving_in_file) {
    ...
    if (fs::create_directories(gen.parent_path())) {
      std::cout << "Structure de dossiers '" << gen.parent_path() << "' créée.\n";
    }
